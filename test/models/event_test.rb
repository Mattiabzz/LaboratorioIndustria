...
class EventTest < ActiveSupport::TestCase
  ...
  NUM_TESTS.times do |i|
    ...
    test "data inizio maggiore di data fine n=#{i}" do
      @event = crea_dati
      @event.data_inizio = Faker::Time.between(from: DateTime.now+5.days, to: DateTime.now + 10.days)
      @event.data_fine = @event.data_inizio - Faker::Number.between(from: 1, to: 30).days
      assert_not @event.valid?
  end
